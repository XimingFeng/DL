...
* 1st term :math:`E_{z \sim Q_{\phi} (z|x)}(\log p_{\theta}(x|z))`: represent reconstruction likehood
* 2nd term :math:`KL[Q_{\phi} (z|x) || p_{\theta} (z)]`: ensure that our learned distribution Q is similar to the prior distribution :math:`p_{\theta} (z)`
